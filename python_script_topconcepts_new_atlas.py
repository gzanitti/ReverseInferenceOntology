...
print(f'Running parameters: id_region={id_region}, n_folds={n_folds}, resample={resample}, random_state={random_state}, frac_sample={frac}')

start_time = datetime.datetime.now()
print(f'Starting at {start_time}')
if os.path.isfile(f'reverse_inference_results/neuro_paper_ri_topconcepts_probs_region{id_region}_{n_folds}folds_no_resample.hdf'):
...
if os.path.isfile(f'reverse_inference_results/neuro_paper_ri_topconcepts_probs_region{id_region}_{n_folds}folds_no_resample.hdf'):
    ...
    try:
        with nl.scope as e:
            ...
            e.term_prob[e.cp, e.fold, e.PROB[e.cp, e.fold]] = (
                (
                    e.filtered_terms[e.d, e.cp]
                ) // (
            ...
            e.no_term_prob[e.cp, e.fold, e.PROB[e.cp, e.fold]] = (
                (
                    e.filtered_terms[e.d, e.cp]
                ) // (
            ...
            e.termProbBF[e.t, e.f, e.bf, e.p, e.pn] = (
                e.term_prob[e.t, e.f, e.p] &
                e.no_term_prob[e.t, e.f, e.pn] &
                (e.bf == (e.p / e.pn))
            )

            res = nl.query((e.t, e.f, e.bf, e.p, e.pn), e.termProbBF[e.t, e.f, e.bf, e.p, e.pn])
            end_time = datetime.datetime.now()
            print(f'{id_region}: {end_time - start_time}')

            pss = res.as_pandas_dataframe()
            pss.to_hdf(f'reverse_inference_results/neuro_paper_ri_topconcepts_probs_region{id_region}_{n_folds}folds_no_resample.hdf', key=f'results')
