...
parser.add_argument("--difumo_components", nargs='?', type=str, default=None)
...
difumo_components = value.difumo_components
...
if difumo_components is not None:
    print(f'    Loading DiFuMo atlas with {difumo_components} components')
...
if not os.path.isfile('./removed_min_img.csv'):
    removed = ns_doc_folds[ns_doc_folds.fold == i]
    removed.to_csv('removed_min_img.csv', index=False)

ns_doc_folds = ns_doc_folds[ns_doc_folds.fold != i]
...
try:
    with nl.scope as e:
        ...
        if difumo_components is not None:
            mean_bf.to_hdf(f'./results/ibc_results_region_{id_region}_difumo_{difumo_components}comps.hdf', key='results')
        else:
